 ...
 * Ver : CCMiniCloud Framework 2.12.1 ALL RIGHTS RESERVED BY nzc816 (wechat)
 */
...
class MultiModel extends Model {
	...
	static _getWhere(where, mustPID = true) {
		if (mustPID) {
			if (typeof (where) == 'string' || typeof (where) == 'number') {
				...
				//where.and._pid=util.getProjectId()
      }
		}
		return where;
	}
	...
	static async getOne(where, fields = '*', orderBy = {}, mustPID = false) {
		//where = MultiModel._getWhere(where, mustPID);
	...
	static async editForms(where, formName, objName, hasImageForms, mustPID = false) {
		where = MultiModel._getWhere(where, mustPID);
