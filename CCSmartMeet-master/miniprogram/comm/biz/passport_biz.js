...
class PassportBiz extends BaseBiz {
  ...
  // 是否登录
  static isLogin(that = null) {
    let isLogin = cacheHelper.get(constants.CACHE_IS_LOGIN);
    if (that) that.setData({ isLogin });
    return isLogin;
  }
  static login(that = null) {
    cacheHelper.set(constants.CACHE_IS_LOGIN, true);
    if (that)
      that.setData({
        isLogin: true,
      });
  }
  static logout(that = null) {
    cacheHelper.remove(constants.CACHE_IS_LOGIN);
    if (that)
      that.setData({
        isLogin: false,
      });
  ...
  static loginStatusHandler(method, status) {
    ...
    if (status == 0) content = "您的注册正在审核中，暂时无法使用此功能！";
      ...
      content =
        "您的注册审核未通过，暂时无法使用此功能；请在个人中心修改资料，再次提交审核！";
    ...
    let res = await cloudHelper
      ...
      .then((result) => {
        ...
        if (
          ...
          result.data.token &&
          (result.data.token.status == 1 || result.data.token.status == 10)
        ) {
          ...
          helper.isDefined(result.data.token) &&
          result.data.token &&
          (result.data.token.status == 0 ||
            result.data.token.status == 8 ||
            result.data.token.status == 9)
        ) {
          ...
          wx.showModal({
            ...
            success(result) {
              if (result.confirm) {
                let url =
                  pageHelper.fmtURLByPID("/pages/my/reg/my_reg") +
                  "?retUrl=back";
                wx.navigateTo({ url });
          ...
          wx.showModal({
            ...
            success(result) {
              if (result.confirm) {
                let retUrl = encodeURIComponent(
                  pageHelper.getCurrentPageUrlWithArgs()
                );
                let url =
                  pageHelper.fmtURLByPID("/pages/my/reg/my_reg") +
                  "?retUrl=" +
                ...
                if (len == 1) {
                  let url = pageHelper.fmtURLByPID(
