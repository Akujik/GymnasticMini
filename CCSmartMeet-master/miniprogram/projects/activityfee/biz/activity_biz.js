...
class ActivityBiz extends BaseBiz {
  ...
  static async cancelMyActivityJoin(activityJoinId, callback, isPay = false) {
    let cb = async () => {
      try {
        ...
        await cloudHelper
          .callCloudSumbit("activity/my_join_cancel", params, opts)
          .then((res) => {
            if (isPay)
              pageHelper.showModal(
                "已取消，所缴纳费用将在1-5分钟内原路退还",
                "温馨提示",
                callback
              );
            else pageHelper.showModal("取消成功", "温馨提示", callback);
          });
      } catch (err) {
        console.log(err);
      }
  ...
  static async setShopIdOptions(that, isAdmin) {
    const list = await cloudHelper.callCloud(
      isAdmin ? "admin/shop_list" : "shop/list"
    );
    if (isAdmin) {
