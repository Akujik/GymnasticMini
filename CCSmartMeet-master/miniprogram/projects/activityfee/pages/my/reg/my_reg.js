...
Page({
  ...
  bindSubmitTap: async function (e) {
    try {
      ...
      let pic = await cloudHelper.transTempPicOne(
        this.data.formPic,
        "user/",
        "",
        false
      ...
      await cloudHelper
        .callCloudSumbit("passport/register", data, opts)
        .then((result) => {
          if (
            result &&
            ...
            let callback = () => {
              if (this.data.retUrl == "back") wx.navigateBack();
              else if (this.data.retUrl)
                wx.redirectTo({
                  url: this.data.retUrl,
                });
              else wx.reLaunch({ url: "../index/my_index" });
            };
            ...
            else pageHelper.showSuccToast("注册成功", 1500, callback);
          }
        });
    } catch (err) {
      console.error(err);
