...
Page({
  ...
  _loadDetail: async function () {
    ...
    let activity = await cloudHelper.callCloudData(
      "admin/activity_detail",
      params,
      opt
    );
    if (!activity) {
    ...
    if (
      !Array.isArray(activity.ACTIVITY_JOIN_FORMS) ||
      activity.ACTIVITY_JOIN_FORMS.length == 0
    )
    ...
    this.setData({
      ...
      formShopId: activity.ACTIVITY_SHOP_ID,
  ...
  bindFormSubmit: async function () {
    ...
    try {
      ...
      // 先修改，再上传
      await cloudHelper
        .callCloudSumbit("admin/activity_edit", data)
        .then((res) => {
          // 更新列表页面数据
          let node = {
