...
#### US-4: 钱包冻结状态下的候补限制 (Priority: P1)

当用户钱包处于冻结状态时，系统需要限制用户加入候补队列，冻结期间无法参与任何候补或补课操作，确保冻结政策的有效执行。

**Why this priority**: 冻结期间限制候补功能是冻结政策的重要组成部分，确保冻结用户无法通过候补渠道获得课程名额。

**Independent Test**: 设置用户冻结状态，尝试加入候补队列，验证冻结限制是否正确执行。

**Acceptance Scenarios**:

1. **Given** 用户钱包处于冻结状态，**When** 查看满员课程，**Then** "加入候补"按钮显示为禁用状态，提示"课卡处于冻结阶段，暂无法候补"
2. **Given** 用户钱包处于冻结状态，**When** 点击"加入候补"按钮，**Then** 系统拒绝添加到候补队列并显示冻结状态提示
3. **Given** 用户已加入候补后被冻结，**When** 系统检查状态，**Then** 自动从所有候补队列中移除该用户，并发送通知告知候补资格已取消
4. **Given** 用户尝试补课操作，**When** 系统检测冻结状态，**Then** 拒绝创建补课记录并提示冻结状态
5. **Given** 冻结期间有名额释放，**When** 系统发送候补通知，**Then** 跳过冻结状态用户，通知下一个可用候补用户
- 前面有用户取消时，候补位置实时更新为连续排位
- 用户点击"取消候补"弹出确认框，确认后从候补队列移除

#### US-5: 补课管理 (Priority: P1)
