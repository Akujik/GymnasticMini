...
#### US-003: 冻结期间私教课程限制 (Priority: P0)
**作为** 系统
**当** 用户钱包处于冻结状态时
**需要** 限制用户访问私教课程服务和提交咨询申请
**确保** 冻结政策的有效执行

**验收标准**：
- Given 用户钱包处于冻结状态，When 访问私教课程页面，Then 系统显示"课卡处于冻结阶段"提示，不显示私教课程列表
- Given 用户点击"咨询预约"按钮，When 系统检测冻结状态，Then 显示冻结提示，不弹出客服二维码弹窗
- Given 用户尝试提交私教咨询，When API验证冻结状态，Then 拒绝创建咨询记录并返回冻结状态错误
- Given 用户查看冻结提示，When 点击"申请取消冻结"入口，Then 跳转到钱包管理页面显示冻结详情
...
#### 2.3.2 冻结状态检查成功标准 (SC-005至SC-007)
- **SC-005**: 冻结期间私教课程页面访问限制准确率100%，显示"课卡处于冻结阶段"提示
- **SC-006**: 冻结期间咨询操作限制准确率100%，API返回冻结状态错误信息
- **SC-007**: "申请取消冻结"入口显示准确率100%，正确跳转至钱包管理页面

#### 2.3.3 FR-040匹配成功标准 (SC-008至SC-010)
- **SC-008**: 私教课程4维标签匹配准确率100%（等级+年龄+性别+类型任一维度不匹配则不显示）
- **SC-009**: 私教课程匹配度计算准确率>95%，匹配详情说明清晰易懂
- **SC-010**: 私教课程个性化价格显示准确率100%，基于4维匹配和用户权益动态计算

#### 2.3.4 FR-042仅浏览模式成功标准 (SC-011至SC-016)
- **SC-011**: 私教课程仅浏览模式执行准确率100%（无直接在线预约功能）
- **SC-012**: "预约咨询"按钮转化率>30%，咨询表单提交成功率>98%
- **SC-013**: 咨询响应及时率>95%（运营人员24小时内响应）
- **SC-014**: 线下私教预约录入成功率>98%，预约信息准确率100%
- **SC-015**: 私教咨询到预约转化率>20%，用户满意度>90%
...
- **冻结期间咨询**: 用户冻结期间咨询私教课程，系统拒绝咨询申请并提示冻结状态，引导用户申请取消冻结
