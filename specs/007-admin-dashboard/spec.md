...
**Status**: Draft
**MVP**: MVP-5
**Dependencies**: MVP-1 (001-user-identity-system), MVP-3 (005-payment-integration), MVP-4 (006-wallet-system)
**Input**: "Build a comprehensive admin dashboard for operations management, including user management, wallet adjustment, course management, data analytics, and audit logging with simplified payment records."
...
**Why this priority**: 权限管理是后台系统的基础，必须优先实现以确保系统安全。

**Independent Test**: 使用不同角色账号登录，验证权限控制是否正确。

**Acceptance Scenarios**:

1. **Given** 管理员使用正确的用户名和密码登录，**When** 点击登录，**Then** 成功进入管理后台首页
2. **Given** 运营人员登录，**When** 进入系统，**Then** 只能看到用户管理、钱包调整、课程管理等功能
3. **Given** 教练角色登录，**When** 进入系统，**Then** 只能看到自己负责的课程和学员信息，可以标记出勤状态
4. **Given** 财务角色登录，**When** 进入系统，**Then** 只能查看钱包和交易记录，无法进行修改
5. **Given** 用户使用错误密码登录，**When** 点击登录，**Then** 显示"用户名或密码错误"提示
...
**Why this priority**: 用户管理是运营后台的核心功能，用户详情页的消课对账记录是运营最重要的工作页面。

**Independent Test**: 创建测试用户，验证信息展示和管理功能是否正确。

**Acceptance Scenarios**:

1. **Given** 运营进入用户管理页面，**When** 页面加载完成，**Then** 显示所有用户列表（包含用户名、手机号、注册时间等）
2. **Given** 运营搜索用户"张三"，**When** 点击搜索，**Then** 显示符合条件的用户信息
3. **Given** 运营点击某用户详情，**When** 进入用户详情页头部，**Then** 显示微信昵称+手机号+OpenID+注册时间，钱包余额¥1000.00（欠费标红），[调整钱包]按钮
4. **Given** 运营查看用户详情页Tab1"消课与对账记录"，**When** 页面加载完成，**Then** 显示统计卡片：累计充值¥5000.00，累计消费¥4800.00，剩余余额¥200.00，总消课数24节课
5. **Given** 运营查看消课记录表格，**When** 查看数据，**Then** 显示所有记录：时间/档案姓名/课程名称/消课金额/交易类型/余额/备注，支持按时间范围/档案/交易类型筛选
6. **Given** 运营查看用户详情页Tab2"预约与档案"，**When** 页面加载完成，**Then** 显示档案卡片列表，每个档案可展开显示最近5条预约记录
7. **Given** 运营查看用户详情，**When** 用户欠费，**Then** 用户信息标红显示，右上角显示"催缴"按钮
8. **Given** 运营选择某用户并点击"冻结"，**When** 确认操作，**Then** 该用户无法登录小程序，系统记录操作日志
...
**Why this priority**: 钱包调整是运营工作的重要内容，必须确保操作安全和数据准确。

**Independent Test**: 创建测试用户并调整余额，验证余额变更和交易记录是否正确。

**Acceptance Scenarios**:

1. **Given** 运营选择用户并填写调整金额+100元，**When** 填写调整原因"线下充值"、收款方式"微信"、订单号"wx123"，**Then** 用户余额增加100元
2. **Given** 运营填写调整金额-50元，**When** 填写调整原因"线下退款"、收款方式"银行转账"、订单号"bank456"，**Then** 用户余额减少50元
...
### User Story 4 - 体验课续课跟进 (Priority: P0)
...
1. **Given** 用户完成体验课，**When** 运营查看跟进列表，**Then** 该用户显示在"待跟进"列表中（红色高亮）
2. **Given** 运营联系用户了解续课意向，**When** 选择"已跟进"，**Then** 记录联系时间和跟进结果（有意向/考虑中/无意向）
3. **Given** 用户有意向转为正式课，**When** 运营点击"续课办理"，**Then** 跳转到用户详情页可调整钱包余额
4. **Given** 运营查看转化率报表，**When** 选择时间范围，**Then** 显示体验课付费转化率和续课转化率数据
5. **Given** 运营标记某用户为"已完成跟进"，**When** 更新状态，**Then** 该用户从"待跟进"列表中移除
...
**Why this priority**: 课程管理是运营后台的基础功能，支持灵活的课程安排。

**Independent Test**: 创建测试课程，验证课程管理功能是否正确。

**Acceptance Scenarios**:

1. **Given** 运营创建新课程，**When** 填写课程信息，**Then** 系统创建新课程记录
2. **Given** 运营修改课程信息，**When** 保存修改，**Then** 课程信息更新成功
3. **Given** 运营删除有预约的课程，**When** 确认删除，**Then** 系统提示"该课程有X人预约，确认删除将自动处理退款/恢复余额"
4. **Given** 课程被删除，**When** 查看操作日志，**Then** 记录删除操作和相关信息
5. **Given** 教练登录查看自己负责的课程，**When** 进入课程列表，**Then** 只显示自己负责的课程
...
1. **Given** 教练登录并进入课程页面，**When** 选择今日课程，**Then** 显示所有预约学员列表
2. **Given** 教练标记某学员为"已到课"，**When** 确认标记，**Then** 系统记录出勤状态和时间
3. **Given** 教练标记某学员为"缺勤"，**When** 确认标记，**Then** 系统记录缺勤状态，不涉及任何扣费
4. **Given** 学员请假，**When** 教练标记为"请假"，**Then** 系统记录请假状态
5. **Given** 运营查看出勤统计，**When** 选择时间范围，**Then** 显示出勤率和缺勤统计报表
...
1. **Given** 运营创建新课程，**When** 设置课程标签，**Then** 可以设置等级范围["L1+", "L2"]、年龄段"4-6岁"、技能类型["平衡", "柔韧"]等
2. **Given** 运营修改课程标签，**When** 更新等级范围，**Then** 系统重新计算该课程与学员的匹配度
3. **Given** 运营查看标签匹配日志，**When** 选择某课程，**Then** 显示该课程的历史匹配记录和匹配分数分布
4. **Given** 系统检测到标签异常，**When** 运营查看报告，**Then** 显示"标签不一致"警告，如课程等级与年龄段不匹配
5. **Given** 运营批量更新课程标签，**When** 选择多个课程，**Then** 支持批量设置相同的标签模板
...
**Why this priority**: 学员标签是课程匹配的核心数据，虚拟年龄功能需要运营审核和调整。

**Independent Test**: 设置学员标签和虚拟年龄，验证课程匹配结果是否符合预期。

**Acceptance Scenarios**:

1. **Given** 运营查看学员详情，**When** 查看标签信息，**Then** 显示年龄标签、等级、性别、发展标签、权益标签等完整信息
2. **Given** 运营调整学员发展标签，**When** 从"兴趣"改为"专业"，**Then** 系统重新计算该学员的课程推荐
3. **Given** 家长申请虚拟年龄设置，**When** 运营审核，**Then** 可以批准或拒绝设置，并记录审核原因
4. **Given** 虚拟年龄差异过大(5岁vs 12岁)，**When** 运营处理，**Then** 系统提示需要教练评估确认
5. **Given** 教练提交虚拟年龄调整申请，**When** 运营审核，**Then** 可以根据教练建议调整学员虚拟年龄
...
1. **Given** 运营创建价格规则，**When** 设置规则名称"儿童团课L1-L4老用户价"，**Then** 配置客户类型"老用户"、人群"儿童"、课程"团课"、等级"L1-L4"、折扣率1.0
2. **Given** 运营设置亲友权益价格，**When** 配置规则，**Then** 设置折扣率0.6，课程显示"🎉亲友专享6折"标签
3. **Given** 运营查看价格规则冲突，**When** 检测冲突，**Then** 系统提示"规则优先级设置建议"，避免价格计算混乱
4. **Given** 运营测试价格规则，**When** 输入测试条件，**Then** 显示计算结果"原价180元，折扣价108元，规则：亲友价"
...
**Why this priority**: 候补管理是预约系统的重要组成部分，需要运营监控和异常处理。

**Independent Test**: 模拟候补队列场景，验证管理功能是否正常。

**Acceptance Scenarios**:

1. **Given** 课程有候补队列，**When** 运营查看，**Then** 显示候补人数列表、候补轮次、截止时间等信息
2. **Given** 候补用户过期未确认，**When** 运营查看，**Then** 显示过期原因和处理状态
3. **Given** 候补通知发送失败，**When** 运营处理，**Then** 可以手动重新发送通知或联系用户
4. **Given** 候补系统异常，**When** 运营干预，**Then** 支持手动调整候补顺序和分配名额
5. **Given** 运营查看候补统计，**When** 选择时间范围，**Then** 显示候补成功率、平均等待时间等指标
...
**Why this priority**: 私教课采用线下预约模式，需要运营人员手动录入和管理。

**Independent Test**: 模拟私教课预约，验证录入功能是否正常。

**Acceptance Scenarios**:

1. **Given** 家长线下预约私教课，**When** 运营录入，**Then** 填写学员档案、教练、时间、价格等信息创建预约记录
2. **Given** 私教课价格线下协商，**When** 运营录入，**Then** 可以填写实际协商价格并备注价格确认信息
3. **Given** 教练联系方式变更，**When** 运营更新，**Then** 修改教练的电话、微信等联系信息
4. **Given** 用户咨询私教课，**When** 运营查看，**Then** 显示咨询记录和跟进状态
5. **Given** 私教课需要调整，**When** 运营处理，**Then** 支持修改时间、更换教练等操作，并通知用户
...
**Why this priority**: 补课补偿是课时管理的重要功能，需要运营监控和异常处理。

**Independent Test**: 创建补课补偿记录，验证管理功能是否正常。

**Acceptance Scenarios**:

1. **Given** 学员有补课补偿记录，**When** 运营查看，**Then** 显示补偿时长、来源、状态等信息
2. **Given** 补偿课时即将过期，**When** 运营查看，**Then** 系统显示预警提醒，可联系用户使用
3. **Given** 补偿使用异常，**When** 运营处理，**Then** 可以手动调整补偿记录或添加备注
4. **Given** 用户累积大量补偿课时，**When** 运营查看，**Then** 系统提醒主动联系用户安排补课
5. **Given** 运营查看补偿统计，**When** 选择时间范围，**Then** 显示补偿发放量、使用率等指标
...
**Why this priority**: 数据统计是运营决策的重要依据，必须提供准确和及时的数据分析。

**Independent Test**: 生成测试数据，验证统计图表和数据是否正确显示。

**Acceptance Scenarios**:

1. **Given** 运营进入数据看板，**When** 页面加载完成，**Then** 显示今日/本周/本月的关键业务指标
2. **Given** 运营查看用户增长趋势，**When** 选择月度视图，**Then** 显示过去12个月的用户增长曲线图
3. **Given** 运营查看收入统计，**When** 选择按课程维度，**Then** 显示各课程的收入贡献占比
4. **Given** 运营查看欠费用户统计，**When** 点击欠费用户数，**Then** 跳转到欠费用户列表
5. **Given** 运营查看出勤率统计，**When** 选择教练和课程，**Then** 显示该教练的出勤率数据
...
用户详情页提供完整的消课对账记录查询功能，支持多维度筛选、统计卡片展示和Excel导出，便于运营进行财务核对。

**Why this priority**: 消课对账是运营日常工作中最重要的功能，必须提供清晰、准确、便捷的查询体验。

**Independent Test**: 创建包含多种交易记录的测试用户，验证对账功能是否准确。

**Acceptance Scenarios**:

1. **Given** 运营进入用户详情页Tab1"消课与对账记录"，**When** 页面加载完成，**Then** 显示4个统计卡片：累计充值、累计消费、剩余余额、总消课数
2. **Given** 运营查看消课记录表格，**When** 查看记录，**Then** 显示混合的充值、消课、调整记录，按时间倒序排列
3. **Given** 运营筛选"交易类型=充值"，**When** 应用筛选，**Then** 表格只显示充值记录，包含线下充值记录
4. **Given** 运营筛选"档案=张三"，**When** 应用筛选，**Then** 表格只显示张三相关的所有消课记录
5. **Given** 运营筛选"时间范围=2025年10月"，**When** 应用筛选，**Then** 表格只显示10月份的所有交易记录
6. **Given** 运营点击"导出对账Excel"按钮，**When** 下载完成，**Then** 获得包含当前筛选条件下所有记录的Excel文件
...
**Why this priority**: 预约档案管理是运营了解用户课程参与情况的重要功能。

**Independent Test**: 创建包含多个学员档案的用户，验证档案管理和预约历史查询功能。

**Acceptance Scenarios**:

1. **Given** 运营进入用户详情页Tab2"预约与档案"，**When** 页面加载完成，**Then** 显示该用户的所有学员档案卡片
2. **Given** 运营查看档案卡片"张三(5岁,L2)"，**When** 点击展开，**Then** 显示张三最近5条预约记录
3. **Given** 展开的预约记录显示状态"已完成"，**When** 查看详情，**Then** 显示预约时间、课程名称、消课状态等完整信息
4. **Given** 运营查看"已完成"的预约记录，**When** 点击查看，**Then** 显示对应的消课记录链接
5. **Given** 运营点击"查看完整预约历史"按钮，**When** 跳转到新页面，**Then** 显示该用户所有档案的所有预约记录（混合显示）

---

### User Story 10 - 操作日志与审计 (Priority: P0)

系统自动记录所有后台操作的日志，运营人员可以查询和审计操作记录。
...
1. **Given** 运营调整用户钱包余额，**When** 操作完成，**Then** 系统记录操作人、时间、操作内容、金额等信息
2. **Given** 运营冻结用户账号，**When** 操作完成，**Then** 系统记录冻结原因和操作人员信息
...
运营在后台创建/编辑课程时，可以为课程设置多维度标签（等级范围/年龄范围/性别/类型等），这些标签用于小程序的智能匹配算法，确保推荐给合适的学员。

**Why this priority**: 课程标签是智能匹配系统的核心数据，直接影响用户体验和预约转化率，必须由运营精确配置。

**Independent Test**: 创建不同标签的课程，验证小程序匹配结果是否符合预期。

**Acceptance Scenarios**:

1. **Given** 运营创建新课程，**When** 进入标签设置页面，**Then** 可以设置等级范围（如选择"L1,L2"表示L1和L2都可报）、年龄范围、性别要求、课程类型
2. **Given** 运营编辑课程标签，**When** 修改年龄标签为"5-6岁"，**Then** 保存后小程序端该课程的匹配年龄范围立即更新
3. **Given** 运营设置课程等级范围为["L1+", "L2"]，**When** 保存成功，**Then** 系统正确识别跨级匹配规则，L1.5学员可以看到该课程
4. **Given** 运营为课程添加技能标签"柔韧训练、协调训练"，**When** 保存成功，**Then** 小程序端显示对应技能标签
5. **Given** 运营设置课程为"热门"程度，**When** 保存成功，**Then** 该课程在小程序推荐排序中获得更高权重
6. **Given** 运营查看课程标签列表，**When** 进入标签管理页面，**Then** 可以看到所有课程的标签配置，支持批量修改和导出

---

### User Story 12 - 学员标签管理 (Priority: P0)

运营在后台可以手动为学员设置等级和发展标签，年龄标签由系统自动计算，权益标签根据消费记录自动更新。这些标签用于课程的智能匹配和推荐。

**Why this priority**: 学员标签的准确性直接影响课程推荐的准确性，需要运营及时维护和更新。

**Independent Test**: 修改学员标签，验证小程序推荐课程的变化是否符合预期。

**Acceptance Scenarios**:

1. **Given** 运营在学员详情页，**When** 修改等级为L3，**Then** 学员在小程序看到的课程列表立即更新为适合L3的课程
2. **Given** 运营设置学员发展标签为"专业班"，**When** 保存成功，**Then** 系统优先推荐专业类型的课程给该学员
3. **Given** 运营查看学员标签历史，**When** 进入标签变更记录，**Then** 显示所有标签修改的时间、操作人、修改原因
4. **Given** 学员年龄达到下一个年龄段，**When** 系统自动计算，**Then** 年龄标签自动更新，推荐课程相应调整
5. **Given** 学员首次购买正式课，**When** 系统检测，**Then** 权益标签从"new_user"自动更新为"old_user"
6. **Given** 运营需要批量调整学员等级，**When** 使用批量操作功能，**Then** 可以选择多个学员一次性调整等级，系统记录操作日志
7. **Given** 学员标签设置异常，**When** 运营查看标签分析报告，**Then** 系统显示可能的标签错误和修正建议

---

### User Story 13 - 标签匹配规则配置 (Priority: P1)

高级运营人员可以配置标签匹配算法的权重参数，优化推荐效果。系统提供匹配效果分析，帮助运营调整算法参数。
...
1. **Given** 高级运营进入算法配置页面，**When** 查看当前权重设置，**Then** 显示基础匹配60%+发展匹配25%+权益匹配10%+时间匹配5%的分配
2. **Given** 运营调整发展匹配权重到30%，**When** 保存配置，**Then** 新的匹配算法立即生效，推荐结果相应调整
3. **Given** 运营查看匹配效果报告，**When** 选择时间范围，**Then** 显示预约转化率、匹配准确率、用户满意度等关键指标
4. **Given** 系统检测到匹配效果下降，**When** 生成预警报告，**Then** 运营收到通知并查看具体的优化建议
5. **Given** 运营测试新的匹配规则，**When** 使用A/B测试功能，**Then** 可以对比不同规则的效果差异
6. **Given** 运营需要回滚算法配置，**When** 点击恢复历史配置，**Then** 系统支持恢复到任意历史版本的配置

---

### User Story 14 - 价格规则管理 (Priority: P0)

运营在后台可以管理价格规则，包括创建/编辑/删除不同客户类型和等级的价格，支持价格规则的批量导入和调整。

**Why this priority**: 价格规则是系统定价的核心逻辑，直接影响营收和用户体验，必须由运营精确管理。

**Independent Test**: 创建不同类型的价格规则，验证小程序端价格显示是否准确更新。

**Acceptance Scenarios**:

1. **Given** 运营在后台"价格管理"页面，**When** 点击"新增价格规则"，**Then** 可以选择客户类型(old_user/new_user/friend)/人群类型(child/adult)/课程类型(group/long_term/private_1v1/private_1v2)/等级范围并设置价格
2. **Given** 运营编辑"儿童团课L1-L4老用户价"，**When** 修改价格为200元，**Then** 小程序课程列表自动更新为老用户显示200元价格
3. **Given** 运营设置"亲友权益折扣率"为0.60，**When** 亲友用户查看团课，**Then** 显示基础价格的6折价格，并显示"🎉亲友专享6折"标签
4. **Given** 运营批量导入价格规则，**When** 上传Excel文件，**Then** 系统验证数据格式，成功导入规则并显示导入结果报告
5. **Given** 运营查看价格规则列表，**When** 筛选"团课-老用户"，**Then** 显示所有团课的老用户价格规则，支持按生效日期排序
6. **Given** 运营需要调整所有老用户价格，**When** 使用批量操作功能，**Then** 可以选择多条规则一次性调整，系统记录操作日志

---

### User Story 15 - 客户权益设置 (Priority: P0)

运营在后台可以手动为学员设置客户类型（老用户/新用户/亲友权益），系统自动根据首次报课日期判断用户类型。
...
1. **Given** 运营在学员详情页，**When** 修改客户类型为"亲友权益"，**Then** 学员在小程序看到的团课价格自动变为6折价格
2. **Given** 运营查看学员的registration_date为2024-10-01，**When** 系统自动计算，**Then** 显示客户类型为"老用户"，界面显示"基于首次报课日期自动计算"
3. **Given** 运营需要批量调整学员权益，**When** 使用批量设置功能，**Then** 可以选择多个学员一次性设置权益，系统提供撤销操作
4. **Given** 运营设置学员为"新用户"，**When** 该学员查看课程，**Then** 显示新用户价格，体验课价格为200元且检查OpenID限制
5. **Given** 运营查看学员价格历史，**When** 点击查看价格记录，**Then** 显示该学员所有预约的价格明细，包含基础价格、折扣、实付价格
6. **Given** 运营需要验证客户类型设置，**When** 查看客户类型分析报告，**Then** 显示老用户/新用户/亲友权益的分布统计和转化率数据

---

## Edge Cases

- **权限冲突**: 用户同时拥有多个角色，如何处理权限？ → 以最高权限为准，但记录所有角色标识
...
- **wallet_adjustment**: 钱包调整记录实体，记录运营调整操作
...
- **waitlist_management**: 候补管理实体，记录候补队列运营处理信息
...
CREATE TABLE attendance_records (
  ...
  marked_by INT NOT NULL COMMENT '标记人（教练ID）',
...
**版本**: v1.1.0
**状态**: Draft
**更新内容**: 整合最终需求 - 标签管理、虚拟年龄、价格规则、候补管理、私教课线下预约、补课补偿等运营后台功能