# Feature Specification: Course Display and Booking System (RuoYi架构版)

**Feature Branch**: `002-course-display-and-booking`
**Created**: 2025-11-17
**Status**: Draft
**MVP**: MVP-2A
**Dependencies**: MVP-1 (001-user-identity-system)
**架构**: RuoYi-Vue-Pro (Spring Boot + MyBatis-Plus + Vue3)
**Input**: "Build a course display and booking system where parents can browse courses, view course details, book classes for themselves or their children, manage their bookings, and cancel/request leave."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - 混合课程显示与智能匹配 (Priority: P1)

家长打开小程序"预约Tab"(小程序共有4个Tab:首页/预约/课程/我的),系统显示成人和儿童课程混合列表，按后台设置顺序排列。无论用户是否有档案，都显示基于课程维度的分类标签（如课程类型、难度等级等）供用户筛选浏览。有档案的用户可看到基于档案信息的个性化推荐，无档案用户看到浮动提示引导创建档案。

**Why this priority**: 混合显示最大化课程曝光度，课程维度标签提供精准筛选能力，引导式体验提升档案创建率，必须优先实现。

**Independent Test**: 验证课程列表是否正确混合显示，课程维度分类标签功能是否正常工作，浮动提示是否按条件显示。

**Acceptance Scenarios**:

1. **Given** 用户无学员档案，**When** 进入"预约Tab"，**Then** 显示成人和儿童混合课程列表，按后台设置顺序排列，同时显示浮动提示条"填写学员资料，获取最合适的课程！"
2. **Given** 用户有档案且为5岁L2男性学员，**When** 进入"预约Tab"，**Then** 显示混合课程列表，按L3以下100%匹配规则(等级+年龄+性别都要满足)智能匹配并显示推荐提示"为5岁L2男性学员推荐：适合课程"
3. **Given** 用户有档案且为20岁成人，**When** 进入"预约Tab"，**Then** 显示混合课程列表，按L3及以上100%匹配规则(等级+年龄+性别都要满足)智能匹配并显示推荐提示"为成人推荐：适合课程"
4. **Given** 用户查看课程列表，**When** 点击"体验课"分类标签，**Then** 筛选显示仅体验课类型课程，可恢复到混合显示
5. **Given** 用户查看课程列表，**When** 点击"正式课"分类标签，**Then** 筛选显示仅正式课类型课程，可恢复到混合显示
6. **Given** 用户查看课程列表，**When** 点击"全部"分类标签，**When** 查看列表，**Then** 恢复混合显示所有课程类型
7. **Given** 用户有档案但点击任何预约操作，**When** 系统检测，**Then** 正常执行预约流程（无需再次验证档案）
8. **Given** 用户无档案且点击任何预约操作，**When** 系统检测，**Then** 拦截操作并提示"请先创建学员档案，获取您的课程"，引导创建档案

---

### User Story 2 - 智能匹配固定班课程 (Priority: P1)

家长在"预约Tab"点击"固定班"入口,系统基于当前档案的标签智能匹配并显示适合的长期课程(3个月学期),按匹配度和热门程度综合排序。

**Why this priority**: 固定班是核心收入来源,智能标签匹配提升预约转化率,必须与灵活排课同等优先级。

**Independent Test**: 点击"固定班"入口,验证是否按匹配度显示所有标签匹配的长期课程。

**Acceptance Scenarios**:

1. **Given** 用户在"预约Tab"且当前档案为5岁L2男性学员, **When** 点击"固定班"入口, **Then** 进入固定班课程列表页,按3维硬匹配规则,显示所有等级包含L2且年龄4-6岁且性别的长期课程,按匹配度排序
2. **Given** 用户查看固定班列表, **When** 点击某个匹配度95分的固定班课程, **Then** 显示课程详情+匹配详情页面(包含3维硬匹配分数:等级100分+年龄100分+性别100分,软标签排序:发展80分)
3. **Given** 固定班已满员, **When** 查看该课程, **Then** 显示"已满员"标签、"匹配度XX分"和"加入候补"按钮
4. **Given** 固定班未满员但匹配度较低, **When** 查看该课程, **Then** 显示"剩余X个名额"、"匹配度XX分"和"立即预约"按钮,可点击查看匹配详情
5. **Given** 当前档案为长训班用户, **When** 浏览固定班, **Then** 系统优先显示long_term类型课程,并在匹配度计算中增加发展标签权重
6. **Given** 无完全匹配的固定班, **When** 查看固定班列表, **Then** 显示"部分匹配课程",降低匹配标准显示相近课程,并标注"建议联系客服评估适合度"

---

### User Story 3 - 基于年龄自动识别的预约流程 (Priority: P1)

家长点击课程卡片进入详情页,查看课程信息(时间/地点/教练/价格/剩余名额等)。系统基于档案年龄自动识别成人/儿童类型，简化预约流程，无需用户手动选择。

**Why this priority**: 自动识别减少用户操作步骤，提升用户体验，避免选择错误导致的预约问题，必须P1实现。

**Independent Test**: 完整走通"选课程→查看详情→自动识别→预约→支付(体验课)→预约成功"流程。

**Acceptance Scenarios**:

1. **Given** 用户点击课程卡片，**When** 进入详情页，**Then** 显示课程信息：课程名称/时间/地点/教练/价格/剩余名额/课程介绍
2. **Given** 当前档案为5岁儿童，**When** 查看课程详情页，**Then** 系统自动识别为儿童类型，预约确认时显示"为儿童档案XXX预约本课程"
3. **Given** 当前档案为20岁成人，**When** 查看课程详情页，**Then** 系统自动识别为成人类型，预约确认时显示"为成人档案XXX预约本课程"
4. **Given** 用户在详情页点击"立即预约"，**When** 弹出确认弹窗，**Then** 弹窗显示"为XXX预约本课程"，系统自动基于档案信息进行3维硬匹配验证
5. **Given** 用户确认预约正式课，**When** 系统执行验证，**Then** 3维硬匹配验证(等级+年龄+性别)+时间冲突检查→预约成功，任一条件不匹配→提示具体原因
6. **Given** 用户确认预约体验课，**When** 系统检查体验次数，**Then** 未体验过+3维硬匹配通过→跳转支付页，已体验过→提示"每个微信号仅可体验一次"
7. **Given** 用户切换到不同年龄的档案，**When** 预约同一课程，**Then** 系统自动更新类型识别，确认弹窗反映新档案的类型特征

---

### User Story 4 - 时间冲突检测 (Priority: P0)

系统必须在用户预约课程前检测是否有时间冲突，若有冲突则提示并禁止预约，确保用户不会同时预约两节重叠时间的课程。

**Why this priority**: 避免用户同时预约两节重叠时间的课程，保证预约数据的合理性和运营效率。

**Independent Test**: 创建时间重叠的预约场景，验证系统是否能正确检测冲突并阻止预约。

**Acceptance Scenarios**:

1. **Given** 用户已预约周三15:00-16:00的课程A，**When** 尝试预约周三15:30-16:30的课程B，**Then** 系统提示"时间冲突，请先取消《课程A》后再预约"并禁止预约
2. **Given** 用户已预约周三15:00-16:00的课程A，**When** 尝试预约周三16:00-17:00的课程B，**Then** 预约成功(时间刚好衔接，无重叠)
3. **Given** 用户切换到其他档案，**When** 预约重叠时间的课程，**Then** 预约成功(不同档案独立管理，时间不冲突)
4. **Given** 用户已预约周五19:00-20:00的课程C，**When** 尝试预约周五20:00-21:00的课程D，**Then** 预约成功(时间连续不重叠)
5. **Given** 用户尝试预约当天已过去的课程时间，**When** 系统检测，**Then** 预约失败并提示"课程时间已过期"

---

### User Story 5 - 查看我的课程 (Priority: P1)

家长在"课程Tab"查看当前档案的所有课程,包括即将上的课程(当期课程)和已上过的课程(历史课程)。

**Why this priority**: 用户需要清楚看到自己的课程安排,P1必须实现。

**Independent Test**: 预约课程后,在"课程Tab"验证课程是否正确显示。

**Acceptance Scenarios**:

1. **Given** 用户进入"课程Tab", **When** 查看当期课程列表, **Then** 显示所有状态为"未开课"的预约记录,按时间升序排列
2. **Given** 用户进入"课程Tab", **When** 切换到"已完成"Tab, **Then** 显示所有已上过的课程(状态为"已完成")
3. **Given** 用户在当期课程列表, **When** 点击某节课, **Then** 进入课程详情页,显示课程信息+教练+地点+取消按钮
4. **Given** 用户切换档案, **When** 刷新"课程Tab", **Then** 显示新档案的课程列表(数据隔离)

---

### User Story 5 - 取消预约/请假 (Priority: P1)

家长在"课程Tab"点击"取消"按钮,对灵活排课可以直接取消,对固定班改为"请假"逻辑。

**Why this priority**: 用户需要灵活管理课程,取消功能必须实现。

**Independent Test**: 分别测试"灵活排课取消"和"固定班请假"两种场景。

**Acceptance Scenarios**:

1. **Given** 用户查看灵活排课的课程详情, **When** 点击"取消预约"按钮, **Then** 显示确认弹窗"确认取消预约?"
2. **Given** 用户确认取消且距开课>6小时, **When** 系统处理, **Then** 体验课通过微信原路退款,正式课课时费退回钱包,名额释放,通知候补队列(如有)
3. **Given** 用户确认取消但距开课<6小时, **When** 系统处理, **Then** 提示"开课前6小时内无法取消,可联系客服"
4. **Given** 用户查看固定班课程详情, **When** 点击"请假"按钮, **Then** 显示确认弹窗"请假后课时不扣除,名额将释放给候补"
5. **Given** 用户确认请假且距开课>6小时, **When** 系统处理, **Then** 课时不扣除,生成"待补课"记录,名额释放,通知候补队列
6. **Given** 用户确认请假但距开课<6小时, **When** 系统处理, **Then** 课时扣除(算旷课),不生成待补课记录,提示"请假失败"
7. **Given** 用户本月已请假3次, **When** 第4次请假, **Then** 提示"本月请假次数已达上限(3次)"

---

### User Story 6 - 长期固定班预约 (Priority: P0)

用户可选择长期固定班或单次预约，两者有不同的扣费和取消规则。固定班报名时立即从钱包扣除课时费锁定名额。

**Why this priority**: 核心业务逻辑，影响预约和扣费流程，必须优先实现。

**Independent Test**: 可独立测试长期固定班和单次预约的扣费逻辑。

**Acceptance Scenarios**:

1. **Given** 用户选择长期固定班，**When** 排队成功，**Then** 立即从钱包扣除课时费，状态变为"待上课"
2. **Given** 固定班用户开课前7小时请假，**When** 确认请假，**Then** 开课前6小时外可申请退费，课时不扣除，生成"待补课"记录，释放名额
3. **Given** 固定班用户开课前5小时请假，**When** 确认请假，**Then** 开课前6小时内无法退费，课时扣除(算旷课)，不生成待补课记录
4. **Given** 用户选择单次预约，**When** 预约成功，**Then** 立即扣除课时费，状态变为"待上课"
5. **Given** 固定班用户连续请假4次，**When** 系统检测，**Then** 自动通知运营释放班级名额
6. **Given** 用户钱包余额不足预约固定班，**When** 系统检查，**Then** 显示提示"您的钱包余额不足,请联系教务老师进行钱包充值"
7. **Given** 固定班用户在"课程-已约"查看课程，**When** 查看未来课程，**Then** 显示未来一周内的所有已预约课程,不限制数量

---

### User Story 7 - 钱包冻结状态下的课程浏览和预约限制 (Priority: P1)

当用户钱包处于冻结状态时，系统需要提供清晰的状态提示和限制预约操作，同时为用户提供取消冻结的便捷入口。

**Why this priority**: 冻结状态直接影响用户体验，必须明确告知用户当前状态并提供解决方案，避免用户困惑和操作失败。

**Independent Test**: 模拟用户冻结状态，验证界面提示是否正确显示，预约操作是否被正确限制，取消冻结入口是否可用。

**Acceptance Scenarios**:

1. **Given** 用户钱包处于冻结状态，**When** 进入"预约Tab"，**Then** 正常显示课程列表，但在页面顶部显示冻结状态提示条"课卡处于冻结阶段"
2. **Given** 用户钱包处于冻结状态，**When** 点击任何课程的"立即预约"按钮，**Then** 显示弹窗"课卡处于冻结阶段，暂时无法预约课程"，并提供"申请取消冻结"按钮
3. **Given** 用户点击"申请取消冻结"按钮，**When** 系统处理，**Then** 显示确认弹窗"确定要申请取消冻结吗？取消后将恢复预约权限"
4. **Given** 用户确认取消冻结，**When** 系统处理成功，**Then** 钱包状态变为激活，页面提示"冻结已取消，您现在可以正常预约课程"
5. **Given** 用户钱包冻结但查看课程详情，**When** 进入课程详情页，**Then** 正常显示课程信息，但预约按钮显示为"课卡冻结中"的禁用状态
6. **Given** 管理员在后台冻结用户钱包，**When** 用户在小程序进行任何操作，**Then** 下次API调用时立即检测到冻结状态并显示相应提示

---

### User Story 8 - 课时费计算规则 (Priority: P1)

系统通过4步顺序判断逻辑计算课时费，确保价格计算的准确性和一致性。

**Why this priority**: 价格计算是核心业务逻辑，直接影响用户体验和收入，必须清晰准确。

**Independent Test**: 测试不同人员属性、用户属性、权益类型和课程标签组合的价格计算。

**Acceptance Scenarios**:

1. **Given** 系统计算课时费，**When** 执行第1步判断人员属性，**Then** 判断约课人员是"儿童"还是"成人"，对应不同的价格表
2. **Given** 约课人员为儿童，**When** 执行第2步判断用户属性，**Then** 判断是"老用户"还是"新用户"，匹配对应的儿童价格表
3. **Given** 约课人员为成人，**When** 执行第2步判断用户属性，**Then** 跳过用户属性判断，成人不需要区分新老用户
4. **Given** 用户有"亲友权益"，**When** 执行第3步判断权益类型，**Then** 所有课程(除长训班外)都为6折
5. **Given** 通过前3步确定价格表，**When** 执行第4步匹配课程标签和等级，**Then** 先匹配"课程标签"，再匹配"等级标签"，最终获得课时费
6. **Given** 5岁儿童老用户无亲友权益预约L2兴趣班，**When** 系统计算，**Then** 通过4步判断获得对应价格表的课时费
7. **Given** 20岁成人有亲友权益预约正式课，**When** 系统计算，**Then** 跳过第2步，直接应用第3步亲友权益6折，再匹配课程标签和等级

---

### Edge Cases

- **网络异常**: 预约时网络中断，如何处理？ → 显示Toast"网络异常，请稍后重试"，使用乐观锁机制
- **并发预约**: 多人同时预约最后一门课，如何处理？ → 使用数据库乐观锁，先到先得，失败提示"课程已被抢光"
- **课程时间冲突**: 用户预约时间冲突的课程，如何处理？ → 前端检查冲突并提示"与现有预约时间冲突"
- **教练取消课程**: 教练临时取消课程，如何处理？ → 系统自动取消所有相关预约，发送通知
- **课程信息变更**: 课程时间或价格变更，如何处理？ → 系统通知所有预约用户，提供免费取消选项
- **候补通知失败**: 取消预约成功后候补通知失败，如何处理？ → 系统重试3次，仍失败则记录日志并通知运营
- **候补用户不响应**: 候补通知发送后用户不响应，如何处理？ → 10分钟内未确认自动过期，顺位通知下一位候补用户
- **候补队列处理**: 取消预约后如何触发候补通知？ → 取消预约成功→立即释放名额→异步触发候补通知→10秒内完成通知

#### 钱包冻结状态相关
- **用户冻结状态**: 用户钱包被冻结时尝试预约课程，如何处理？ → 显示"课卡处于冻结阶段"提示，禁止预约操作，提供"申请取消冻结"入口
- **冻结期间浏览**: 用户冻结期间浏览课程列表，如何处理？ → 允许浏览课程，但点击任何预约操作都显示冻结状态提示
- **冻结申请确认**: 用户申请冻结时，如何处理？ → 弹窗告知"一年只能冻结一次，一次3个月"，需要用户二次确认
- **管理员冻结**: 管理员在后台冻结用户钱包，如何处理？ → 立即生效，用户在下次操作时被限制并收到通知
- **冻结状态检查**: 预约API如何检查冻结状态？ → 在预约流程开始时检查钱包状态，冻结状态直接返回错误提示

#### 浮动提示和档案引导相关
- **无档案用户浏览**: 用户没有档案但浏览课程列表，如何处理？ → 允许浏览混合课程，显示浮动提示引导创建档案
- **浮动提示关闭**: 用户关闭浮动提示，如何处理？ → 隐藏提示但记录关闭状态，下次启动应用时重新显示
- **预约操作拦截**: 无档案用户点击预约操作，如何处理？ → 拦截操作并显示"请先创建学员档案，获取您的课程"提示

#### 年龄自动识别相关
- **年龄边界处理**: 学员年龄正好18岁，如何识别类型？ → 按照标准定义为成人类型（18岁及以上为成人）
- **虚拟年龄使用**: 学员有虚拟年龄设置时，如何处理匹配？ → 档案信息基于实际数据，3维硬匹配中的年龄维度使用虚拟年龄
- **类型识别错误**: 系统类型识别与用户期望不符，如何处理？ → 预约确认时显示识别依据，允许用户联系客服调整
- **多档案类型切换**: 用户切换到不同年龄类型档案，如何处理？ → 立即更新类型识别，预约确认反映新档案特征

---

## Requirements *(mandatory)*

### Functional Requirements

#### Core Requirements

- **FR-001**: 系统必须基于当前档案的多维度标签(年龄/等级/性别/发展标签/权益标签)智能匹配课程,按匹配度排序显示
- **FR-002**: 系统必须支持按周切换日期(如本周/下周),每周7天独立显示,切换日期时重新计算匹配度
- **FR-003**: 系统必须显示课程的完整标签信息(年龄段/等级/性别/类型/技能/发展特征),标签数据来自course_tags表
- **FR-004**: 系统必须显示课程详情页的所有字段+标签匹配详情:课程名称/类型/适合年龄/适合等级/上课时间/上课地点/教练信息/课程价格/剩余名额/课程介绍/匹配分数/匹配原因
- **FR-005**: 系统必须限制体验课预约次数(每个微信OpenID仅可预约一次),并在标签匹配中过滤已购买过的体验课
- **FR-006**: 系统必须执行3维硬匹配规则:所有学员都必须通过等级+年龄+性别的强制验证(3维全部匹配才显示)，类型标签降级为软标签仅用于推荐排序 (根据Q4,Q11,Q16,Q19更新)
- **FR-007**: 系统必须支持跨级标签匹配(L1+/L2+等格式),正确判断学员等级是否在课程等级范围内
- **FR-040**: 系统必须实施3维硬匹配白名单验证(FR-040):等级维度+年龄维度+性别维度,任一维度不匹配则课程不显示 (根据Q4,Q11,Q16,Q19更新)
- **FR-008**: 系统必须检测时间冲突(同一档案不能在同一时间预约两节课),并在标签匹配前预先过滤
- **FR-009**: 系统必须在预约正式课时检查钱包余额,余额不足时允许欠费预约,记录标签匹配日志到tag_match_log表
- **FR-010**: 系统必须计算并显示匹配分数(0-100分),基于100%匹配逻辑(用户标签完全包含课程标签),非占比权重计算
- **FR-011**: 系统必须提供匹配详情说明,让用户理解为什么推荐某个课程
- **FR-012**: 系统必须支持匹配度低于60分的课程预约,但需提示用户匹配度较低并建议联系客服
- **FR-013**: 系统必须在"课程Tab"显示当期课程和历史课程,并支持档案切换,显示课程时的匹配度信息
- **FR-014**: 系统必须支持灵活排课的取消预约(开课前6小时),取消时记录标签匹配日志
- **FR-015**: 系统必须支持固定班的请假逻辑(开课前6小时,课时不扣除,生成待补课记录),请假时影响后续标签匹配
- **FR-016**: 系统必须记录连续请假次数,连续请假4次时自动通知运营人员跟进处理
- **FR-017**: 系统必须在取消预约/请假后释放名额并通知候补队列,重新计算候补用户的标签匹配度
- **FR-018**: 系统必须支持固定班的插班申请(已开课但未满员的课程),插班时显示标签匹配度供运营参考
- **FR-019**: 系统必须自动更新课程状态(可报名/已满员/已开课/已结束),状态变化影响标签匹配结果

#### 钱包冻结状态检查需求

- **FR-041**: 系统必须在所有预约API开始时检查用户钱包状态,冻结状态下禁止预约操作
- **FR-042**: 系统必须在用户钱包冻结时在课程列表页面显示冻结状态提示条
- **FR-043**: 系统必须在用户点击预约操作时检查冻结状态,冻结时显示弹窗提示并提供取消冻结入口
- **FR-044**: 系统必须支持用户在小程序内申请取消冻结,包含确认流程
- **FR-045**: 系统必须在冻结期间禁用所有预约相关按钮,但允许用户继续浏览课程信息
- **FR-046**: 系统必须在冻结状态解除后立即恢复用户的预约权限

#### 时间冲突检测相关
- **FR-020**: 系统必须在用户预约课程前检测是否有时间冲突
- **FR-021**: 系统必须支持时间重叠的精确判断(包含开始时间和结束时间的交叉)
- **FR-022**: 系统必须支持不同档案间的独立预约(不同档案的时间不冲突)
- **FR-023**: 系统必须在检测到时间冲突时提供明确的冲突信息和解决方案
- **FR-024**: 系统必须支持时间衔接的预约(如15:00-16:00和16:00-17:00)
- **FR-025**: 系统必须检查课程时间的有效性(不允许预约已过期的时间)

#### 长期固定班预约相关
- **FR-033**: 系统必须支持长期固定班预约，预约成功后状态为"排队中"
- **FR-034**: 系统必须在长期固定班开课前6小时自动扣除课时费
- **FR-035**: 系统必须支持固定班开课前7小时取消(计入请假次数)，释放临时名额
- **FR-036**: 系统必须在固定班开课前6小时内禁止取消(自动扣费并生成补课权益)
- **FR-037**: 系统必须连续请假4次时自动通知运营释放班级名额
- **FR-038**: 系统必须在课程-已约界面为固定班用户仅显示未来2节课
- **FR-039**: 系统必须存储长期固定预约记录(recurring_booking表)，包含开始/结束日期和状态

#### 价格计算相关
- **FR-026**: 系统必须根据学员的customer_type(老用户/新用户/亲友权益)和等级(L1-L8)动态计算课程价格
- **FR-027**: 系统必须在课程列表和详情页显示实际价格(考虑折扣后的个性化价格)
- **FR-028**: 系统必须为亲友权益用户在团课价格上显示"🎉亲友专享6折"标签
- **FR-029**: 系统必须在预约确认页面显示价格明细(基础价格/折扣/实付价格)
- **FR-030**: 系统必须支持多维度定价规则:客户类型/人群类型/课程类型/等级范围
- **FR-031**: 系统必须支持固定价格课程(体验课200元)和动态价格课程(基于用户权益计算)
- **FR-032**: 系统必须存储价格规则(pricing_rule表):规则名称/客户类型/人群类型/课程类型/等级范围/每小时价格/折扣率

#### Data Requirements

- **FR-027**: 系统必须存储课程的所有字段:课程名/类型/时间段/地点/教练ID/价格/最大人数/最少人数/候补上限/课程介绍
- **FR-028**: 系统必须存储课程标签信息(course_tags表):等级范围(JSON数组)/年龄范围/性别要求/课程类型/技能类型(JSON数组)/课程强度/主教老师/助教支持/热门程度/目标人群/定价类别/基础价格/定价策略
- **FR-029**: 系统必须存储预约记录:预约ID/档案ID/课程ID/预约时间/状态(待上课/已完成/已取消/请假)/匹配分数/匹配详情
- **FR-030**: 系统必须存储标签匹配日志(tag_match_log表):档案ID/课程ID/是否匹配/匹配分数/匹配详情/匹配规则类型/算法版本/创建时间
- **FR-031**: 系统必须存储价格规则(pricing_rule表):规则名称/客户类型/人群类型/课程类型/等级范围/每小时价格/折扣率/生效日期/是否启用
- **FR-032**: 系统必须存储补课记录:补课ID/原预约ID/档案ID/生成时间/状态(待补课/已补课)
- **FR-033**: 系统必须存储教练信息:教练ID/姓名/照片/等级/擅长/介绍/评分/授课时长
- **FR-034**: 系统必须记录学员的完整标签信息:年龄标签/等级/性别/发展标签/权益标签/关系标签/自动计算标签/首次报课日期/客户类型
- **FR-035**: 系统必须存储预约类型标识(single单次/recurring长期固定)
- **FR-036**: 系统必须存储预约状态(排队中/待上课/已完成/已取消/候补中/候补失败)
- **FR-037**: 系统必须存储补课相关信息:是否为补课/原始预约ID/补课截止日期
- **FR-038**: 系统必须存储签到相关信息:签到状态/签到时间/管理员ID
- **FR-039**: 系统必须存储连续请假次数(固定班用)
- **FR-040**: 系统必须存储长期固定预约记录:开始日期/结束日期/状态/连续请假次数

### Key Entities

- **course**: 课程实体，包含基础课程信息
  - 核心属性: id, name, description, price, max_capacity, min_capacity, location, introduction, end_date
  - 业务规则: 课程基础信息与标签信息分离存储; 支持主教练+代课教练模式; 固定班课程需要设置结束日期

- **course_tags**: 课程标签实体，存储课程的多维度标签信息
  - 核心属性: id, course_id, level_range (JSON数组如["L1+", "L2"]), age_range, gender, course_type, skill_types (JSON数组), intensity_level, main_instructor, has_assistant, popularity
  - 业务规则: 支持跨级标签(L1+等); 多技能标签JSON存储; 热门程度影响推荐排序; 与course一对一关系

- **schedule**: 课程时间表实体，定义课程的具体时间安排
  - 核心属性: id, course_id, start_time, end_time, location, recurring_pattern
  - 业务规则: 支持单次和重复课程安排，固定班学期3个月; 时间冲突检测基于schedule

- **booking**: 预约实体，记录用户的课程预约和匹配信息
  - 核心属性: id, user_id, profile_id, course_id, schedule_id, status, booking_time, match_score, match_details (JSON)
  - 业务规则: 每个预约记录匹配分数和详情; 状态包含待上课/已完成/已取消/请假/旷课; 匹配详情供分析使用

- **tag_match_log**: 标签匹配日志实体，记录匹配算法的执行过程
  - 核心属性: id, profile_id, course_id, is_matched, match_score, match_details (JSON), match_rule, algorithm_version, created_at
  - 业务规则: 每次匹配计算都记录日志; 支持算法版本迭代; 匹配详情JSON存储各维度得分; 用于调试和优化

- **profile**: 学员档案实体，包含学员的标签信息(扩展自MVP-1)
  - 核心属性: id, name, birthday, gender, level, development, privilege, status
  - 业务规则: level影响课程匹配; development标签用于发展匹配; privilege标签影响权益匹配; 自动计算年龄标签

- **instructor**: 教练实体，包含教练信息
  - 核心属性: id, name, avatar_url, bio, specialties, qualifications, rating, teaching_hours
  - 业务规则: 教练可以管理自己的课程，评分由运营手动设置

- **makeup_record**: 补课记录实体，记录固定班请假后的补课需求
  - 核心属性: id, original_booking_id, profile_id, generated_time, status (待补课/已补课)
  - 业务规则: 请假成功后自动生成，无有效期限制; 补课时同样需要标签匹配

---

## Success Criteria *(mandatory)*

- **SC-001**: 用户切换档案后,课程列表在500ms内按标签匹配度重新排序显示,成功率>99%
- **SC-002**: 用户点击课程卡片进入详情页,页面加载时间<1秒,匹配详情显示准确率>99%
- **SC-003**: 用户完成预约流程(选课程→查看匹配度→确认预约),整个流程<2分钟,流程放弃率<10%
- **SC-004**: 3维硬匹配白名单规则执行准确率100%(所有学员必须通过等级+年龄+性别3维强制验证,任一维度不匹配则课程不显示) (根据Q4,Q11,Q16,Q19更新)
- **SC-005**: 跨级标签匹配准确率100%(L1+学员能正确匹配["L1+", "L2"]课程)
- **SC-006**: 标签匹配算法执行时间<100ms,匹配分数计算准确率>95%
- **SC-007**: 体验课限制准确率100%(已体验过的微信号无法再次预约体验课)
- **SC-008**: 时间冲突检测准确率100%(同一时间不允许预约两节课)
- **SC-009**: 取消预约/请假后,名额在10秒内释放并重新计算候补队列匹配度,成功率>95%
- **SC-010**: 课程状态自动更新准确率100%(已满员/已开课等状态实时更新,影响匹配结果)
- **SC-011**: 标签匹配日志记录完整率100%(每次匹配都有详细日志可供分析)
- **SC-012**: 固定班插班申请提交成功率>99%,审核时显示标签匹配度供运营参考
- **SC-013**: "课程Tab"加载当期课程列表时间<1秒,显示课程匹配度信息准确率>99%

---

## Assumptions

- 假设课程数据由运营人员通过Web后台管理
- 假设课程时间安排合理，不会频繁变更
- 假设用户网络环境稳定，能够完成预约操作
- 假设用户了解课程取消规则和时间限制

---

## Out of Scope (MVP-2A 不实现)

以下功能明确排除在MVP-2A之外,留待后续MVP实现:

1. **候补队列管理** → MVP-2B实现
2. **补课功能的完整实现**(仅生成待补课记录,补课选课功能留到MVP-2B)
3. **私教课系统** → MVP-2C实现
4. **微信支付集成** → MVP-3实现(MVP-2A体验课仅模拟支付)
5. **钱包充值功能** → MVP-4实现
6. **课程评价功能** → MVP-5实现
7. **课程分享功能** → MVP-5实现
8. **课程添加到日历** → MVP-5实现
9. **课程提醒通知** → MVP-5实现

---

## Open Questions (optional)

1. **课程图片**: 是否需要上传课程图片?(当前暂不需要,后台可选填)
2. **教练评分**: 教练评分数据来源?(运营手动输入,暂不开放用户评价)
3. **课程推荐**: 是否需要"为你推荐"功能?(MVP-2A不实现,留到后期)
4. **课程搜索**: 是否需要搜索功能?(MVP-2A不实现,筛选即可)

---

## Integration Points

### Dependencies on MVP-1
- **用户身份系统**: 需要MVP-1的用户登录和档案管理功能
- **学员档案**: 预约时需要选择为哪个学员预约
- **JWT认证**: API调用需要使用MVP-1的JWT Token

### Critical Dependencies on Future MVPs
- **MVP-2B (候补与补课)**:
  - 取消预约功能直接触发候补队列机制
  - 请假功能为补课系统提供"待补课"记录输入
  - 需要为MVP-2B预留课程状态变更接口
- **MVP-2C (私教系统)**:
  - 复用预约逻辑和时间冲突检查机制
  - 需要为私教课程预留课程类型扩展
- **MVP-3 (支付系统)**: 为付费课程提供支付功能
- **MVP-5 (运营后台)**: 提供课程管理功能

### API Design Considerations
- **课程状态接口**: 需支持MVP-2B的候补触发机制，集成标签匹配状态
- **预约状态接口**: 需支持MVP-2B的补课记录生成，包含匹配分数和详情
- **时间冲突检查**: 需同时支持团课和私教课类型，基于标签预筛选
- **取消预约接口**: 需异步通知MVP-2B候补系统，重新计算匹配度
- **标签匹配接口**: 新增课程匹配API，支持多维度标签匹配算法
- **价格计算接口**: 新增个性化价格计算API，基于用户权益动态计算价格
- **标签管理接口**: 为MVP-5预留标签管理API接口
- **匹配日志接口**: 支持标签匹配日志查询和分析
- **价格管理接口**: 为MVP-5预留价格规则管理API接口

### 新增API设计

#### GET /api/v1/courses/match
**方法**: GET
**路径**: /api/v1/courses/match
**描述**: 获取符合当前档案标签的课程列表（智能匹配）

**Query Parameters**:
- `profile_id` (required): 档案ID
- `week` (optional): 周数，默认本周
- `limit` (optional): 返回数量限制，默认20
- `include_unmatched` (optional): 是否包含不匹配的课程，默认false

**Response**:
```json
{
  "code": 200,
  "message": "查询成功",
  "data": {
    "profile_tags": {
      "age_tag": "4-5",
      "level": "L2",
      "gender": "male",
      "development": "interest",
      "privilege": "new_user"
    },
    "match_rule": "3d_hard_match_validation",
    "matched_courses": [
      {
        "course_id": 1,
        "name": "基础体操训练班",
        "course_tags": {
          "level_range": ["L1+", "L2"],
          "age_range": "4-5",
          "gender": "both",
          "course_type": "interest",
          "waitlist_capacity": 8
        },
        "match_score": 100.0,
        "validation_result": "whitelist_passed",
        "match_details": {
          "validation_type": "3d_hard_match",
          "overall_match": true,
          "dimension_results": {
            "level_match": { "score": 100, "result": "matched", "reason": "等级L2在范围内" },
            "age_match": { "score": 100, "result": "matched", "reason": "年龄5.2岁符合4-5岁" },
            "gender_match": { "score": 100, "result": "matched", "reason": "性别male符合both要求" }
          },
          "soft_tags": {
            "course_type": "interest",
            "development_match": "matched",
            "recommendation_score": 95
          },
          "validation_summary": {
            "total_dimensions": 3,
            "passed_dimensions": 3,
            "validation_result": "hard_match_passed"
          }
        },
        "schedule": {
          "start_time": "2025-11-01T10:00:00Z",
          "available_spots": 3
        }
      }
    ],
    "pagination": {
      "total": 50,
      "page": 1,
      "limit": 20
    }
  }
}
```

#### GET /api/v1/courses/{id}/tags
**方法**: GET
**路径**: /api/v1/courses/{id}/tags
**描述**: 获取课程的标签信息

**Response**:
```json
{
  "code": 200,
  "message": "查询成功",
  "data": {
    "course_id": 1,
    "level_range": ["L1+", "L2"],
    "age_range": "4-5",
    "gender": "both",
    "course_type": "interest",
    "skill_types": ["flexibility", "coordination"],
    "intensity_level": "medium",
    "main_instructor": "张教练",
    "has_assistant": true,
    "popularity": "normal"
  }
}
```

#### GET /api/v1/profiles/{id}/tags
**方法**: GET
**路径**: /api/v1/profiles/{id}/tags
**描述**: 获取学员的标签信息

**Response**:
```json
{
  "code": 200,
  "message": "查询成功",
  "data": {
    "profile_id": 1,
    "age_tag": "4-5",
    "level": "L2",
    "gender": "male",
    "development": "interest",
    "privilege": "new_user",
    "auto_calculated_tags": {
      "age_tag": "4-5岁",
      "age_range": "4-5",
      "active_status": "active"
    }
  }
}
```

#### GET /api/v1/courses/{id}/price
**方法**: GET
**路径**: /api/v1/courses/{id}/price
**描述**: 获取课程的个性化价格（基于学员档案动态计算）

**Query Parameters**:
- `profile_id` (required): 学员档案ID

**Response**:
```json
{
  "code": 200,
  "message": "查询成功",
  "data": {
    "course_id": 1,
    "profile_id": 1,
    "customer_type": "old_user",
    "audience": "child",
    "course_type": "group",
    "level": "L2",
    "base_price": 180.00,
    "discount_rate": 1.00,
    "final_price": 180.00,
    "price_rule": "儿童团课L1-L4老用户价",
    "price_breakdown": {
      "price_per_hour": 180.00,
      "duration_hours": 1.0,
      "subtotal": 180.00,
      "discount": 0.00,
      "total": 180.00
    },
    "discount_label": null
  }
}
```

**亲友权益示例**:
```json
{
  "code": 200,
  "message": "查询成功",
  "data": {
    "customer_type": "friend",
    "base_price": 180.00,
    "discount_rate": 0.60,
    "final_price": 108.00,
    "price_rule": "儿童团课L1-L4亲友价",
    "price_breakdown": {
      "price_per_hour": 180.00,
      "duration_hours": 1.0,
      "subtotal": 180.00,
      "discount": 72.00,
      "total": 108.00
    },
    "discount_label": "🎉亲友专享6折"
  }
}
```

### Technical Dependencies
- **微信小程序**: 前端展示和用户交互
- **后端API**: 业务逻辑和数据管理
- **数据库**: 数据持久化和查询
- **微信支付**: 未来支付功能的集成（MVP-3）

---

## Appendix *(optional)*

### 课程标签字段枚举

**年龄段** (固定字段):
- `2.5-4岁`
- `4-6岁`
- `6-12岁`
- `12-18岁`
- `18岁以上`

**课程等级** (固定字段):
- `L1`, `L2`, `L3`, `L4`, `L5`, `L6`

**课程类型** (固定字段):
- `团课` (2人以上)
- `体验课` (单次体验)
- `长训课` (一周10小时以上)
- `冬令营`
- `夏令营`

**技能类型** (固定字段):
- `倒立课`
- `空翻课`

---

---

## 技术实现 (RuoYi架构)

### 核心算法实现

#### 3D硬匹配算法Service
```java
@Service
public class CourseMatchingService {

    @Autowired
    private GymCourseMapper courseMapper;

    @Autowired
    private GymStudentProfileMapper profileMapper;

    /**
     * 3维硬匹配算法：等级+年龄+性别
     */
    public CourseMatchingResult findMatchedCourses(Long profileId) {
        GymStudentProfile profile = profileMapper.selectGymStudentProfileByProfileId(profileId);

        // 计算年龄（使用虚拟年龄如果存在）
        Integer age = profile.getVirtualAge() != null ? profile.getVirtualAge() :
                      calculateAge(profile.getBirthday());

        LambdaQueryWrapper<GymCourse> wrapper = new LambdaQueryWrapper<>();

        // 等级匹配
        wrapper.ge(GymCourse::getMinLevel, profile.getSkillLevel())
               .le(GymCourse::getMaxLevel, profile.getSkillLevel());

        // 年龄匹配
        wrapper.le(GymCourse::getMinAge, age)
               .ge(GymCourse::getMaxAge, age);

        // 性别匹配
        wrapper.and(w -> w.isNull(GymCourse::getGenderRestriction)
                      .or().eq(GymCourse::getGenderRestriction, "2")  // 不限
                      .or().eq(GymCourse::getGenderRestriction, profile.getGender()));

        wrapper.eq(GymCourse::getStatus, "0")
               .orderByAsc(GymCourse::getStartTime)
               .orderByDesc(GymCourse::getPopularity);

        List<GymCourse> courses = courseMapper.selectList(wrapper);

        // 计算匹配度分数
        return new CourseMatchingResult(courses.stream()
                .map(course -> new CourseWithScore(course, calculateMatchingScore(profile, course, age)))
                .sorted((a, b) -> b.getScore().compareTo(a.getScore()))
                .collect(Collectors.toList()));
    }

    private Integer calculateMatchingScore(GymStudentProfile profile, GymCourse course, Integer age) {
        int score = 0;

        // 等级匹配分数 (40%权重)
        if (isLevelMatch(profile.getSkillLevel(), course)) {
            score += 40;
        }

        // 年龄匹配分数 (30%权重)
        if (isAgeMatch(age, course)) {
            score += 30;
        }

        // 性别匹配分数 (30%权重)
        if (isGenderMatch(profile.getGender(), course)) {
            score += 30;
        }

        return score;
    }
}
```

#### 时间冲突检测算法
```java
@Service
public class BookingConflictService {

    @Autowired
    private GymBookingMapper bookingMapper;

    /**
     * 检查预约时间冲突
     */
    public boolean hasTimeConflict(Long profileId, LocalDateTime startTime, LocalDateTime endTime) {
        LambdaQueryWrapper<GymBooking> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(GymBooking::getProfileId, profileId)
               .in(GymBooking::getStatus, Arrays.asList("confirmed", "pending"))
               .and(w -> w
                   // 情况1: 新预约开始时间在已有预约时间范围内
                   .between(GymBooking::getStartTime, startTime, endTime)
                   // 情况2: 新预约结束时间在已有预约时间范围内
                   .or().between(GymBooking::getEndTime, startTime, endTime)
                   // 情况3: 新预约完全包含已有预约
                   .or().le(GymBooking::getStartTime, startTime)
                       .ge(GymBooking::getEndTime, endTime));

        Long count = bookingMapper.selectCount(wrapper);
        return count > 0;
    }

    @Transactional(rollbackFor = Exception.class)
    public AjaxResult createBooking(BookingRequest request) {
        // 1. 检查时间冲突
        if (hasTimeConflict(request.getProfileId(), request.getStartTime(), request.getEndTime())) {
            return AjaxResult.error("预约时间与已有课程冲突，请选择其他时间");
        }

        // 2. 执行3D匹配验证
        CourseMatchingResult matchingResult = courseMatchingService.findMatchedCourses(request.getProfileId());
        boolean isMatched = matchingResult.getCourses().stream()
                .anyMatch(course -> course.getCourseId().equals(request.getCourseId()));

        if (!isMatched) {
            return AjaxResult.error("该课程不符合学员的技能等级要求");
        }

        // 3. 检查课程容量
        GymCourse course = courseMapper.selectById(request.getCourseId());
        if (course.getCurrentEnrollment() >= course.getMaxCapacity()) {
            return AjaxResult.error("课程已满员，您可以加入候补列表");
        }

        // 4. 创建预约记录
        GymBooking booking = new GymBooking();
        BeanUtils.copyProperties(request, booking);
        booking.setStatus("pending");
        booking.setCreateTime(LocalDateTime.now());

        return toAjax(bookingMapper.insert(booking));
    }
}
```

### MyBatis-Plus数据访问

#### GymCourse实体
```java
@Data
@EqualsAndHashCode(callSuper = true)
@Accessors(chain = true)
@TableName("gym_course")
public class GymCourse extends BaseEntity {

    private static final long serialVersionUID = 1L;

    @TableId(value = "course_id", type = IdType.AUTO)
    private Long courseId;

    @TableField("course_name")
    @NotBlank(message = "课程名称不能为空")
    private String courseName;

    @TableField("course_type")
    @NotBlank(message = "课程类型不能为空")
    private String courseType;

    @TableField("skill_level_required")
    private String skillLevelRequired;

    @TableField("min_level")
    private String minLevel;

    @TableField("max_level")
    private String maxLevel;

    @TableField("min_age")
    private Integer minAge;

    @TableField("max_age")
    private Integer maxAge;

    @TableField("gender_restriction")
    private String genderRestriction;

    @TableField("max_capacity")
    private Integer maxCapacity;

    @TableField("current_enrollment")
    private Integer currentEnrollment;

    @TableField("start_time")
    private LocalDateTime startTime;

    @TableField("end_time")
    private LocalDateTime endTime;

    @TableField("base_price")
    private BigDecimal basePrice;

    @TableField("popularity")
    private String popularity;

    @Version
    private Integer version;

    // 计算字段
    @TableField(exist = false)
    private String ageRangeText;

    @TableField(exist = false)
    private String levelRangeText;

    @TableField(exist = false)
    private Integer availableSlots;
}
```

#### GymCourseMapper
```java
@Mapper
public interface GymCourseMapper extends BaseMapper<GymCourse> {

    /**
     * 根据学员档案查找匹配课程
     */
    List<GymCourse> selectMatchedCoursesByProfile(@Param("profileId") Long profileId);

    /**
     * 根据条件查询课程列表
     */
    List<GymCourse> selectCourseList(GymCourse course);

    /**
     * 更新课程报名人数
     */
    @Update("UPDATE gym_course SET current_enrollment = current_enrollment + #{increment} WHERE course_id = #{courseId}")
    int updateEnrollment(@Param("courseId") Long courseId, @Param("increment") Integer increment);
}
```

### Spring Boot Controller实现

#### GymCourseController
```java
@RestController
@RequestMapping("/gym/course")
public class GymCourseController extends BaseController {

    @Autowired
    private IGymCourseService courseService;

    @Autowired
    private CourseMatchingService matchingService;

    @GetMapping("/list")
    @Log(title = "课程列表", businessType = BusinessType.SELECT)
    public TableDataInfo list(GymCourse course) {
        startPage();
        List<GymCourse> list = courseService.selectGymCourseList(course);
        return getDataTable(list);
    }

    @GetMapping("/matched/{profileId}")
    @Log(title = "智能匹配课程", businessType = BusinessType.SELECT)
    public AjaxResult getMatchedCourses(@PathVariable Long profileId) {
        CourseMatchingResult result = matchingService.findMatchedCourses(profileId);
        return AjaxResult.success(result);
    }

    @PostMapping("/book")
    @Log(title = "预约课程", businessType = BusinessType.INSERT)
    public AjaxResult bookCourse(@Validated @RequestBody BookingRequest request) {
        return bookingConflictService.createBooking(request);
    }

    @GetMapping("/{courseId}")
    @Log(title = "课程详情", businessType = BusinessType.SELECT)
    public AjaxResult getInfo(@PathVariable Long courseId) {
        GymCourse course = courseService.selectGymCourseByCourseId(courseId);
        if (course == null) {
            return AjaxResult.error("课程不存在");
        }

        // 转换为VO对象
        CourseVO courseVO = new CourseVO();
        BeanUtils.copyProperties(course, courseVO);

        // 计算可用名额
        courseVO.setAvailableSlots(course.getMaxCapacity() - course.getCurrentEnrollment());

        return AjaxResult.success(courseVO);
    }
}
```

### 小程序端集成

#### 课程列表页面
```javascript
// pages/courses/courses.js
Page({
  data: {
    courses: [],
    loading: false,
    categories: [
      { name: '全部', value: 'all' },
      { name: '体验课', value: 'trial' },
      { name: '正式课', value: 'regular' },
      { name: '团课', value: 'group' },
      { name: '私教课', value: 'private' }
    ],
    currentCategory: 'all',
    currentProfile: null,
    showCreateGuide: false
  },

  onLoad() {
    this.loadCurrentProfile();
    this.loadCourses();
  },

  loadCurrentProfile() {
    const profile = wx.getStorageSync('currentProfile');
    this.setData({
      currentProfile: profile,
      showCreateGuide: !profile
    });
  },

  async loadCourses(category = 'all') {
    if (this.data.loading) return;

    this.setData({ loading: true });

    try {
      let url = '/gym/course/list';

      // 如果有学员档案且选择"全部"，使用智能匹配
      if (this.data.currentProfile && category === 'all') {
        url = `/gym/course/matched/${this.data.currentProfile.profileId}`;
      } else if (category !== 'all') {
        url = `/gym/course/list?courseType=${category}`;
      }

      const response = await this.request({
        url: url,
        method: 'GET'
      });

      if (response.code === 200) {
        this.setData({
          courses: response.data.courses || response.data,
          loading: false
        });
      }
    } catch (error) {
      this.setData({ loading: false });
      wx.showToast({
        title: '加载课程失败',
        icon: 'error'
      });
    }
  },

  // 切换分类标签
  onCategoryTap(e) {
    const category = e.currentTarget.dataset.category;
    this.setData({
      currentCategory: category
    });
    this.loadCourses(category);
  },

  // 预约课程
  async onBookCourse(e) {
    if (!this.data.currentProfile) {
      wx.showModal({
        title: '提示',
        content: '请先创建学员档案，获取您的课程',
        confirmText: '去创建',
        success: () => {
          wx.navigateTo({
            url: '/pages/profiles/create'
          });
        }
      });
      return;
    }

    const courseId = e.currentTarget.dataset.courseId;

    // 显示预约确认弹窗
    wx.showModal({
      title: '预约确认',
      content: `为${this.data.currentProfile.studentName}预约本课程？`,
      confirmText: '确认预约',
      success: async () => {
        await this.doBooking(courseId);
      }
    });
  },

  async doBooking(courseId) {
    try {
      wx.showLoading({ title: '预约中...' });

      const response = await this.request({
        url: '/gym/course/book',
        method: 'POST',
        data: {
          courseId: courseId,
          profileId: this.data.currentProfile.profileId
        }
      });

      wx.hideLoading();

      if (response.code === 200) {
        wx.showToast({
          title: '预约成功',
          icon: 'success'
        });

        // 刷新课程列表
        this.loadCourses(this.data.currentCategory);
      } else {
        wx.showToast({
          title: response.msg || '预约失败',
          icon: 'error'
        });
      }
    } catch (error) {
      wx.hideLoading();
      wx.showToast({
        title: '预约失败',
        icon: 'error'
      });
    }
  }
});
```

### RuoYi管理后台集成

#### Vue3课程管理页面
```vue
<template>
  <div class="app-container">
    <!-- 搜索区域 -->
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true">
      <el-form-item label="课程名称">
        <el-input
          v-model="queryParams.courseName"
          placeholder="请输入课程名称"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="课程类型">
        <el-select v-model="queryParams.courseType" placeholder="请选择课程类型" clearable>
          <el-option label="团课" value="group" />
          <el-option label="体验课" value="trial" />
          <el-option label="私教1v1" value="private_1v1" />
          <el-option label="私教1v2" value="private_1v2" />
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" icon="Search" @click="handleQuery">搜索</el-button>
        <el-button icon="Refresh" @click="resetQuery">重置</el-button>
      </el-form-item>
    </el-form>

    <!-- 按钮区域 -->
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          type="primary"
          plain
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['gymnastics:course:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          type="success"
          plain
          icon="Edit"
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['gymnastics:course:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          type="danger"
          plain
          icon="Delete"
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['gymnastics:course:remove']"
        >删除</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList" />
    </el-row>

    <!-- 表格区域 -->
    <el-table v-loading="loading" :data="courseList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="课程ID" align="center" prop="courseId" width="80" />
      <el-table-column label="课程名称" align="center" prop="courseName" :show-overflow-tooltip="true" />
      <el-table-column label="课程类型" align="center" prop="courseType">
        <template #default="scope">
          <dict-tag :options="course_type" :value="scope.row.courseType" />
        </template>
      </el-table-column>
      <el-table-column label="等级要求" align="center" prop="skillLevelRequired" />
      <el-table-column label="年龄范围" align="center">
        <template #default="scope">
          {{ scope.row.minAge }}-{{ scope.row.maxAge }}岁
        </template>
      </el-table-column>
      <el-table-column label="容量" align="center">
        <template #default="scope">
          {{ scope.row.currentEnrollment }}/{{ scope.row.maxCapacity }}
        </template>
      </el-table-column>
      <el-table-column label="价格" align="center" prop="basePrice">
        <template #default="scope">
          ¥{{ scope.row.basePrice }}
        </template>
      </el-table-column>
      <el-table-column label="状态" align="center" prop="status">
        <template #default="scope">
          <dict-tag :options="sys_normal_disable" :value="scope.row.status" />
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" class="small-padding fixed-width">
        <template #default="scope">
          <el-button
            size="mini"
            type="text"
            icon="Edit"
            @click="handleUpdate(scope.row)"
            v-hasPermi="['gymnastics:course:edit']"
          >修改</el-button>
          <el-button
            size="mini"
            type="text"
            icon="Delete"
            @click="handleDelete(scope.row)"
            v-hasPermi="['gymnastics:course:remove']"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>

    <!-- 分页区域 -->
    <pagination
      v-show="total > 0"
      :total="total"
      :page.sync="queryParams.pageNum"
      :limit.sync="queryParams.pageSize"
      @pagination="getList"
    />
  </div>
</template>

<script setup name="Course">
import { listCourse, getCourse, delCourse, addCourse, updateCourse } from "@/api/gymnastics/course";

const { proxy } = getCurrentInstance();

const courseList = ref([]);
const open = ref(false);
const loading = ref(true);
const showSearch = ref(true);
const ids = ref([]);
const single = ref(true);
const multiple = ref(true);
const total = ref(0);
const title = ref("");

const data = reactive({
  form: {},
  queryParams: {
    pageNum: 1,
    pageSize: 10,
    courseName: null,
    courseType: null,
  },
});

const { queryParams, form } = toRefs(data);

/** 查询课程列表 */
function getList() {
  loading.value = true;
  listCourse(queryParams).then(response => {
    courseList.value = response.rows;
    total.value = response.total;
    loading.value = false;
  });
}

/** 搜索按钮操作 */
function handleQuery() {
  queryParams.value.pageNum = 1;
  getList();
}

/** 重置按钮操作 */
function resetQuery() {
  proxy.resetForm("queryRef");
  handleQuery();
}

/** 新增按钮操作 */
function handleAdd() {
  reset();
  open.value = true;
  title.value = "添加课程";
}

/** 修改按钮操作 */
function handleUpdate(row) {
  reset();
  const _courseId = row.courseId || ids.value;
  getCourse(_courseId).then(response => {
    Object.assign(form, response.data);
    open.value = true;
    title.value = "修改课程";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["courseRef"].validate(valid => {
    if (valid) {
      if (form.courseId != null) {
        updateCourse(form).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addCourse(form).then(response => {
          proxy.$modal.msgSuccess("新增成功");
          open.value = false;
          getList();
        });
      }
    }
  });
}

getList();
</script>
```

**创建人**: [产品经理]
**最后更新**: 2025-11-17
**版本**: v2.0.0
**更新内容**: 技术栈重构为RuoYi架构 (Spring Boot + MyBatis-Plus + Vue3)
**状态**: Draft